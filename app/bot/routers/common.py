...
def _help_for_role(role: str) -> List[str]:
    ...
    if role == "student":
        base += [
            "/student — главное меню студента 🆕",  # Новая команда
            "/register — привязка к ростеру", 
            "/slots — свободные слоты и запись",
            "/submit [task_id] — отправить решение", 
            "/grades — мои оценки", 
            "/feedback — отзыв"
        ]
    if role in ("ta", "owner"):
        base += [
            "/professor — главное меню преподавателя 🆕",  # Новая команда
            "/register_ta — заявка TA", 
            "/schedule — создать расписание",
            "/myslots — мои слоты", 
            "/myslots_manage — управление слотами"
        ]
    if role == "owner":
        base += [
            "/setrole <tg_id> <role>", 
            "/ta_pending — заявки TA",
            "/impersonate <tg_id|student_code=...>", 
            "/impersonate_off",
            "/dev_user_role <tg_id> <role>", 
            "/dev_user_del <tg_id>"
        ]
    return base
...
async def start(message: Message, users: UsersService):
    ...
    lines = ["👋 Привет! Это учебный ассистент курса физики."]
    ...
    if actor_role == "student":
        lines.extend([
            f"<b>Ваша роль:</b> {actor_role}",
            "",
            "🆕 <b>Новое главное меню:</b> /student",
            "📚 Или работа с неделями: /week", 
            "📥 Отправка решений: /submit [task_id]",
        ...
        lines.extend([
            ...
            "",
            "🆕 <b>Новое главное меню:</b> /professor",
            "📅 Создание расписания: /schedule",
            "🔍 Просмотр слотов: /myslots",
            "⚙️ Управление слотами: /myslots_manage"
        ])
    elif actor_role == "owner":
        lines.extend([
            f"<b>Ваша роль:</b> владелец курса",
            "",
            "🆕 <b>Интерфейсы:</b> /student, /professor",
            "🎭 Для тестов: /impersonate [tg_id]",
            "👥 Управление: /weeks_list + в разарботке"
        ])
    else:
        # Неизвестный пользователь
        lines.extend([
            f"<b>Ваша роль:</b> не определена",
            "",
            "Выберите тип регистрации:",
            "",
            "📚 <b>Студент:</b> /register",
            "👨‍🏫 <b>Преподаватель:</b> /register_ta",
            "",
            "При регистрации вы будете найдены в ростере курса."
        ])
